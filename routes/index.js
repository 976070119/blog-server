...
router.get('/oneArticle', (req, res) => {
  const { _id } = req.query;
  console.log(_id)
  ArticleModel.find({_id}, filter, (err, data) => {
...
router.get('/search', (req, res) => {
  ...
  if(tags) {
    ArticleModel.find({ tags: eval('/tags/i') }, filter, (err, data) => {
      if(err)throw new Error("获取失败!");
      if(data) {
        res.send({ code: 0, data, success: 'true'});
      }
    ...
    ArticleModel.find({content: eval(`/${keywords}/i`)}, filter, (err, data) => {
      if(err)throw new Error("获取失败!");
      if(data) {
        res.send({ code: 0, data, success: 'true'});
      }
...
router.post('/deleteArt', (req, res) => {
  const { _id } = req.body;
  ArticleModel.findByIdAndDelete({_id}), req.body, ((err, data) => {
    if(data) {
      res.send({code: 0, success: 'true'});
    } else {
      res.send({code: 1, success: 'false'});
...
router.post('/modify', (req, res) => {
  ...
  let lastModifyTime = new Date().toLocaleString();
  ArticleModel.findByIdAndUpdate({_id}, req.body, (err, data) => {
    res.send({err, data})
  })
})
